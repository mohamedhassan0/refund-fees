...
AS
   ...
   BEGIN
      ...
      THEN
         BEGIN
            kaucust.kau_send_mail (p_sender      => 'DCSCE.Community@kau.edu.sa',
   ...
   htp.br;
         ...
         cattributes => 'align="center" WIDTH=360 height=50  id="tbx"');   --height=600
      twbkfrmt.p_tablerowopen;
      htp.tableheader ('··≈” ›”«— Ê«· Ê«’·', 'CENTER');
      twbkfrmt.p_tablerowclose;
    
--      twbkfrmt.p_tablerowopen;
--      twbkfrmt.p_tabledataheader (
--         '«·»—Ìœ «·≈·ﬂ —Ê‰Ì : sadad.support@kau.edu.sa');
--      twbkfrmt.p_tablerowclose;
      twbkfrmt.p_tablerowopen;
       htp.tableheader(
         twbkfrmt.f_printanchor (
            curl      => 'mailto:sadad.support@kau.edu.sa',
            ctext     => ('«·»—Ìœ «·≈·ﬂ —Ê‰Ì : sadad.support@kau.edu.sa'),
            ctarget   => '_blank'),
         'center');
      twbkfrmt.p_tablerowclose;
...
--------------------
      /******************************************/
      /*** JavaScript function to validate on  actual time before  submit ***/
 ...
 twbkfrmt.p_PRINTTEXT ('window.onload = function() {');
  twbkfrmt.p_PRINTTEXT ('var myInput = document.getElementById(''IBAN_OWNER'');');
  twbkfrmt.p_PRINTTEXT ('myInput.onpaste = function(e) {');
    twbkfrmt.p_PRINTTEXT ('e.preventDefault();');
  twbkfrmt.p_PRINTTEXT ('}');
  --
   twbkfrmt.p_PRINTTEXT ('var myInput2 = document.getElementById(''IBAN'');');
  ...
  --
    --
   twbkfrmt.p_PRINTTEXT ('var myInput3 = document.getElementById(''ACOUNT'');');
 ...
 twbkfrmt.p_PRINTTEXT ( ' var unicode=e.charCode? e.charCode : e.keyCode');
 
--  twbkfrmt.p_PRINTTEXT ( ' alert( unicode);');
  twbkfrmt.p_PRINTTEXT ( 'if (unicode!=8  && unicode!=32 ){ //if the key isn''t the backspace key (which we should allow)');
--  twbkfrmt.p_PRINTTEXT ('     if (( unicode<48 || unicode>57) && (unicode < 0x0600 || unicode > 0x06FF)) //if not a number or arabic');
twbkfrmt.p_PRINTTEXT ('     if ( unicode < 0x0600 || unicode > 0x06FF  ) //if not a number or arabic'); --( unicode<48 || unicode>57) &&
 ...
 twbkfrmt.p_PRINTTEXT ( ' var unicode=e.charCode? e.charCode : e.keyCode'); 
--  twbkfrmt.p_PRINTTEXT ( ' alert( unicode);');
  twbkfrmt.p_PRINTTEXT ( 'if (unicode!=8  ){ //if the key isn''t the backspace key (which we should allow)');--&& unicode!=32  for space
--  twbkfrmt.p_PRINTTEXT ('     if (( unicode<48 || unicode>57) && (unicode < 0x0600 || unicode > 0x06FF)) //if not a number or arabic');
twbkfrmt.p_PRINTTEXT ('     if ( unicode <48  || unicode > 57  ) //if not a number'); --( unicode<48 || unicode>57) &&
   twbkfrmt.p_PRINTTEXT ( '   return false //disable key press');
 twbkfrmt.p_PRINTTEXT ('  }');
 twbkfrmt.p_PRINTTEXT ('}');
      twbkfrmt.p_PRINTTEXT ('</script>');
...
--                              '40',
--                              '40',
--                              REC_NOT_REVIEWED_DATA.ACCOUNT_NO,cattributes=>'id="ACOUNT" onkeypress="return EnglishOnly(event)"');
      twbkfrmt.p_formtext (
         'ACCOUNT',
         '18',
         '20',
...
--                              '40',
--                              '40',
--                              REC_NOT_REVIEWED_DATA.IBAN_NO,cattributes=>'id="IBAN" onkeypress="return EnglishOnly(event)"');
      twbkfrmt.p_PRINTTEXT(twbkfrmt.F_formtext (
                               'IBAN',
                               '28',
                               '22',
                               SUBSTR(REC_NOT_REVIEWED_DATA.IBAN_NO,3),
                               cattributes   => 'id="IBAN" onkeypress="return EnglishOnly(event)" ')
                            || 'SA');
         twbkfrmt.p_tabledataclose;
         twbkfrmt.p_tablerowclose;
         ...
         twbkfrmt.p_formtext ('NAME',
                              '40',
                              '40',
                              REC_NOT_REVIEWED_DATA.IBAN_OWNER,cattributes=>'id="IBAN_OWNER" onkeypress="return arabicOnly(event)"');
         twbkfrmt.p_tabledataclose;
         twbkfrmt.p_tablerowclose;
         ...
         twbkfrmt.p_printtext (   HTF.formtextareaopen2 (
                                     ...
                                     5,
                                     70,
                                     cattributes   => 'maxlength="50" ID="reason_id"')
         ...
         HTP.formsubmit (
            '',
            ' ”ÃÌ· «·ÿ·»',
            cattributes   => 'onclick="if(!this.form.agreement_check.checked){alert(''ÌÃ» «·„Ê«›ﬁ… ⁄·Ì «·«ﬁ—«— «Ê·« ﬁ»·  ”ÃÌ· «·ÿ·»'');return false}" ');
         twbkfrmt.p_tabledataclose;
         twbkfrmt.p_tablerowclose;
...
--      select length(NAME) - NVL(length(translate(NAME, '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ~`!@#$%^&*()_-+={[}]|\:;<,>.?/',' ') ),0) 
--       INTO V_CHECK_NAME
--       from dual;
--      IF V_CHECK_NAME <>0
--      THEN
...
--      END IF;
      ...
      BEGIN
--         IF ADM.GET_IBAN (trim(ACCOUNT), Bank) <> trim(IBAN)
   ...
   IS
   CURSOR GET_DATA(P_PIDM NUMBER)
   IS
   SELECT *
   FROM SADAD.V_FEES_REFUND A
   WHERE A.PIDM=P_PIDM 
   ORDER BY TERM DESC ;
   
   cursor get_bank_desc(p_bank varchar2)
   is
            SELECT BANK_DESC
           FROM SADAD.BANK_CODE
          WHERE BANK_CODE = p_bank;
   V_STATUS VARCHAR2(100);
   V_RJCT_REASON VARCHAR2(400);
   v_bank_desc varchar2(100);
   V_ROW_EXISTS VARCHAR2(1):='N';
   BEGIN
          /* Check for valid user */
      IF NOT twbkwbis.f_validuser (pidm)
      THEN
         RETURN;
      END IF;
       bwckfrmt.p_open_doc ('kwlkrfnd.P_FEES_TRACKING');
       htp.br;
    FOR DATA_REC IN GET_DATA(pidm) LOOP
     ...
     THEN
         ...
         twbkfrmt.p_tablerowopen;
          ...
          TWBKFRMT.P_TABLEDATAHEADER(   '<font color="#008000" face="Arial" size="4">'
                                                            ||'«·›’· «·œ—«”Ì ·· ﬁœÌ„'
                                                            ||  '</font>');
          TWBKFRMT.P_TABLEDATAHEADER(  '<font color="#008000" face="Arial" size="4">'
                                                            ||'«·»‰ﬂ'
     ...
     V_RJCT_REASON:='';
     twbkfrmt.p_tablerowopen;
      TWBKFRMT.P_TABLEDATA(f_get_desc_fnc('STVTERM',DATA_REC.TERM,30),'center');
      open get_bank_desc(DATA_REC.BANK_NAME);
      fetch get_bank_desc into v_bank_desc;
      close get_bank_desc;
      TWBKFRMT.P_TABLEDATA(v_bank_desc,'center');
      TWBKFRMT.P_TABLEDATA(DATA_REC.ACCOUNT_NO,'center');
      IF DATA_REC.FILE_STATUS=' „ «·’—›'
      THEN
       v_status:=' „ «·’—›';
      ELSIF DATA_REC.AUDIT_USER_1='Y' and DATA_REC.AUDIT_USER_2='Y'
      THEN
        v_status:='Ã«—Ì «· œﬁÌﬁ';
      ELSIF DATA_REC.AUDIT_USER_1='X' OR DATA_REC.AUDIT_USER_2='X'
      THEN
        v_status:=' „ —›÷ «·ÿ·»';
        V_RJCT_REASON:=DATA_REC.USER_NOTS;
      ELSIF DATA_REC.AUDIT_USER_1='N' OR DATA_REC.AUDIT_USER_2='N'
      THEN
        v_status:=' „ —›÷ «·ÿ·»';
      END IF;
      TWBKFRMT.P_TABLEDATA(v_status,'center');
      TWBKFRMT.P_TABLEDATA(V_RJCT_REASON,'center');
      TWBKFRMT.P_TABLEDATA(DATA_REC.SDAD_AMOUNT,'center');
      TWBKFRMT.P_TABLEDATA(DATA_REC.DEDUCT_AMOUNT,'center');
     twbkfrmt.p_tablerowCLOSE;
     V_ROW_EXISTS:='Y';
    ...
    THEN
                 twbkfrmt.p_printmessage (
               twbkfrmt.f_tabledata (
                  '<font color="RED" face="Arial" size="4"> ·Ì” ·œÌﬂ ÿ·»«  ··„—«Ã⁄…</font>'),
               2);
    END IF;
   twbkwbis.p_closedoc (curr_release);
   END P_FEES_TRACKING;
